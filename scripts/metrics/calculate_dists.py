...
class DISTS(torch.nn.Module):
    def __init__(self, load_weights=True):
        ...
        if load_weights:
            weights = torch.load('/home/IR/scripts/metrics/DISTS/DISTS_pytorch/weights.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--folder_gt', type=str, default='/output/datasets/dehaze_derain/benchmark/RainDS/test_syn/HR/') #/output/datasets/dehaze_derain/benchmark/RainDS/test_real/HR/ /output/datasets/dehaze_derain/benchmark/Raindrop/HR/
    parser.add_argument('--folder_restored', type=str, default='/home/IR/results/Ours_rainstreak_true_105k/visualization/rainstreak/')
    args = parser.parse_args()
    # img_list = sorted(glob.glob(osp.join(args.folder_gt, '*.png')))
    # lr_list = sorted(glob.glob(osp.join(args.folder_restored, '*.png')))

    img_list = sorted(glob.glob(osp.join(args.folder_gt, '*')))
    ###################
    img_list.sort(key=lambda x:int(x.split('.')[0].split('/')[-1]))
    ###################

    lr_list = sorted(list(scandir(args.folder_restored, recursive=True, full_path=True)))
    ###################
    lr_list.sort(key=lambda x:int(x.split('x2_Ours')[0].split('/')[-1]))
    ###################
