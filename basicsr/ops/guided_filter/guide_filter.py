...
#         self.box_filter = nn.Conv2d(dim, dim, kernel_size=3, padding=radius, dilation=radius, bias=False, groups=4)
#         self.conv_a = nn.Sequential(nn.Conv2d(dim*2, dim, kernel_size=1, bias=False),
#                                     nn.ReLU(inplace=True),
...
class ConvGuidedFilter(nn.Module):
    def __init__(self, dim, radius=1):
        ...
        self.box_filter = nn.Conv2d(dim, dim, kernel_size=3, padding=radius, dilation=radius, bias=False, groups=4)
        self.conv_a = nn.Sequential(nn.Conv2d(dim*2, dim//16, kernel_size=1, bias=False),
                                    nn.ReLU(inplace=True),
    ...
    def forward(self, x_lr, y_lr, x_hr):
        ...
        mean_x = self.box_filter(x_lr)/N
        ## mean_y
        N_y = self.box_filter(y_lr.data.new().resize_((1, c, 1, 1)).fill_(1.0))
        mean_y = self.box_filter(y_lr)/N_y
        ## cov_xy
        cov_xy = self.box_filter(x_lr * y_lr)/N - mean_x * mean_y
        ## var_x
        var_x  = self.box_filter(x_lr * x_lr)/N - mean_x * mean_x
