...
class Encoder(nn.Module):
    def __init__(self,n_feats = 64, n_encoder_res = 6,scale=4):
        ...
        self.C = nn.Sequential(
            nn.Conv2d(3, n_feats, kernel_size=7, stride=7, padding=0),
            nn.LeakyReLU(0.1, True),
            ...
            nn.LeakyReLU(0.1, True),
            nn.Conv2d(n_feats*2, n_feats*4, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1, True),
            nn.AdaptiveAvgPool2d(1)
        )

        self.mlp = nn.Sequential(
            nn.Linear(n_feats * 4 * 2, n_feats * 4),
            nn.LeakyReLU(0.1, True),
    ...
    def forward(self, x,gt):
        ...
        fea = torch.cat([self.E(x).squeeze(-1).squeeze(-1), self.C(gt).squeeze(-1).squeeze(-1)], dim=1)
        S1_IPR = []
        fea1 = self.mlp(fea)
        S1_IPR.append(fea1)
        return fea1,S1_IPR
...
class DHGMS1(nn.Module):
    def __init__(self,
        ...
        self.E = Encoder(n_feats=64, n_encoder_res=n_encoder_res,scale=scale)
    ...
    def forward(self, x, gt):
        if self.training:
            ...
            dct_feature = dct(IPRS1, norm='ortho')
            ...
            sr = self.G(x, IPRS1, guide_prior, dct_prior)
